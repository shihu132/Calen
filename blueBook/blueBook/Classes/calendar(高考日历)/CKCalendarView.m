...
//        self.backgroundColor = UIColorFromRGB(0xF2F2F2);//所有的表格颜色,除当时时间的表格颜色
        self.backgroundColor = [UIColor whiteColor];
        self.selectedBackgroundColor = UIColorFromRGB(0x88B6DB);//点击表格选中1--31表格的颜色
        
        //所有1--31表格
        self.textColor = UIColorFromRGB(0x393B40);//黑色
        self.selectedTextColor = UIColorFromRGB(0xF2F2F2);//选中1--31表格
    }
    return self;
}
...
- (void)_init:(CKCalendarStartDay)firstDay {
    ...
    self.dateFormatter.dateFormat = @"yyyy月MM日";
    ...
    [prevButton setImage:[UIImage imageNamed:@"左键"] forState:UIControlStateNormal];
    ...
    [nextButton setImage:[UIImage imageNamed:@"右键"] forState:UIControlStateNormal];
...
//        dayOfWeekLabel.backgroundColor =  UIColorFromRGB(0xF2F2F2);
...
//        UIImageView *imageVieWroundFace = [[UIImageView alloc]initWithFrame:CGRectMake(self.width-20 , 10, 20, 20)];
//        imageVieWroundFace.image = [UIImage imageNamed:@"other"];
//        [dateButton addSubview:imageVieWroundFace];
    }
...
- (void)layoutSubviews {
    ...
    CGFloat containerWidth = [UIScreen mainScreen].bounds.size.width;
    ...
    for (UILabel *dayLabel in self.dayOfWeekLabels) {
        ...
        //星期的位置
        dayLabel.frame = CGRectMake(CGRectGetMaxX(lastDayFrame) + CELL_BORDER_WIDTH-0.2, lastDayFrame.origin.y, self.cellWidth, self.daysHeader.frame.size.height);
    ...
    while ([date laterDate:endDate] != date) {
        ...
        CKDateItem *item = [[CKDateItem alloc] init];
            ...
            item.textColor = [UIColor redColor];//当天的日期
...
- (void)_setDefaultStyle {
    ...
    self.backgroundColor = [UIColor whiteColor];//view 背景颜色

    [self setTitleColor:[UIColor blackColor]];
    [self setTitleFont:[UIFont boldSystemFontOfSize:15.0]];

    //星期字体-颜色
    [self setDayOfWeekFont:[UIFont boldSystemFontOfSize:15.0]];
    [self setDayOfWeekTextColor:UIColorFromRGB(0x393B40)];
    [self setDayOfWeekBottomColor:UIColorFromRGB(0xCCCFD5) topColor:[UIColor whiteColor]];

    [self setDateFont:[UIFont boldSystemFontOfSize:15.0f]];
    [self setDateBorderColor:UIColorFromRGB(0xDAE1E6)];//line间距颜色
}
...
- (void)setMonthButtonColor:(UIColor *)color {
    [self.prevButton setImage:[CKCalendarView _imageNamed:@"左键" withColor:color] forState:UIControlStateNormal];
    [self.nextButton setImage:[CKCalendarView _imageNamed:@"右键" withColor:color] forState:UIControlStateNormal];
}
